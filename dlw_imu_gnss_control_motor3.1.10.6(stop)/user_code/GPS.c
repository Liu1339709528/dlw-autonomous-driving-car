...
//1.先确定坐标点
//2.利用坐标点代入逐飞角度计算函数--方向角
//3.IMU-------------------航向角
//4。用方向角-航向角-角度误差
//存GPS坐标点--1.用flash（可以断电）  2.用数组。
        ...
        for(int data=0;data<200;data+=4)
          {
            ...
            Target_point[0][TG]=(double)flash_union_buffer[data].uint32_type+(double)flash_union_buffer[data+1].float_type; //纬(这里似乎要用double类型的数组,因为理论上是整数加上一个float类型的小数部分)
