...
int main (void)
{
    ...
    dlw_gps_init();                                                  // RTK初始化
    zf_gpio_deinit(F3);
    zf_gpio_init(F3, GPI_PULL_DOWN, 0);
    zf_gpio_deinit(F3);
    zf_gpio_init(F2, GPO_PUSH_PULL, 1);
    ...
    for( ; ; )
    {
        ...
        if(GL_IMU_Flag == 1)
        {
           ...
           if((gnns_sum >= 2&&gnns_sum <= 7)&&remote_ch4 != 1792)
          {
              dlw_set_motor_duty(2100);
          }
           else if((gnns_sum >= 12&&gnns_sum <= 15)&&remote_ch4 != 1792)
           {
               dlw_set_motor_duty(2000);
           }
          else if((gnns_sum >= 8&&gnns_sum <= 10) && remote_ch4 != 1792)
          {
              dlw_set_motor_duty(2100);
          }
          else if(remote_ch4 != 1792)
          {
              dlw_set_motor_duty(3500);
          }
          else if(remote_ch4 == 1792)
          {
        ...
        else if(GL_IMU_Flag == 0)
        {
              ...
              if(remote_ch4 == 1792)
              {
                  zf_gpio_set_level(F2, GPIO_LOW);

              }
              else
              {
                  zf_gpio_set_level(F2, GPIO_HIGH);
              }
        }
    }
}
