...
extern uint8_t left;											//左方向发车
extern uint8_t right;											//右方向发车
...
int main (void)
{
    ...
    int gnns_sum = 1;
    uint8 GL_CRC_state = 0;
    int16   encoder_data[2];
    uint8 gnns_sum_flag = 1,i=0;										 //左发车查找坐标标志位,i下标
    uint16  remote_ch4= uart_receiver.channel[3];                    // 遥控器SWB按键数据通道
    ...
    dlw_gps_init();                                                  // RTK初始化
    dlw_nfc_usart_init();											 //NFC串口初始化
    encoder_PID_init(45,0.1,0);											//编码器初始化
    ...
    zf_pit_ms_init(PIT_TIM5, 100, encoder_pit_handler, encoder_data);
    ...
    zf_pit_set_interrupt_priority(PIT_TIM5, INTERRUPT_PRIORITY_2);   // 中断优先级中间
    zf_delay_ms(1000);
    for( ; ; )
    {
      	sudo = ((-encoder_data[0]/1024.0)*(27.0/44.0)*(12.0/30.0)*3.1415926*7.2*2);
//      encoder_contorl_sudo(50,sudo);
//      zf_delay_ms(50);
        key = dlw_key_scan(0);
        remote_ch4 = uart_receiver.channel[3];
        if(GL_IMU_Flag == 1)
        {
           ...
           ips200_show_float(50, 16*12, (gnns_sum), 3, 6);        //显示当前IMU测量角度（相对于发车点是0度）
           //角度变换
           ...
           if (left)
           {
				   ...
				   while(Target_point[0][i] != 0 && gnns_sum_flag == 1)
				   {
					   gnns_sum = i;
					   i++;
				   }
				   if(remote_ch4 != 1792 )
				   {
					  encoder_contorl_sudo(100,sudo);
				   }
				   if(remote_ch4 == 1792 )
				   {
					  encoder_contorl_sudo(0,sudo);
				   }
					 gnns_sum_flag = 0;
	   ...
	   //                  dlw_set_motor_duty(0);
        ...
        else if(GL_IMU_Flag == 0)
        {
              ...
              dlw_servo_control(channel_ptr[0]);                     // 调用修改后的舵机控制函数
              dlw_moto_control(channel_ptr[1]);
              ips200_show_string(70, 0, "remote control");
              GL_CRC_state = GL_CRC();
              dlw_nfc_usartdata_process();
        }
    }
}
...
{
    (void)event;
    zf_encoder_get_count(ENCODER_TIM2, (((int16 *)ptr) + 0));
...
void NFC_uart_handler (uint32 event, void *ptr)
{
    ...
    {
        uint8 data_temp = 0;
        zf_uart_query_byte(UART_2, &data_temp);
        zf_fifo_write_element(fifo_ptr, data_temp);
    }
}
